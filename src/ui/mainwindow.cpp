...
void MainWindow::formToUser(User& user) const
{
    ...
    for (const auto& field : m_userFormFields) {
        ...
        else if (field.fieldName == "password") user.setPasswordHash(hashPasswordSecure(value.toString()));  // ← Хэшируем!
...
QString MainWindow::checkValidForUser(const User& user)
{
    // === ОБЯЗАТЕЛЬНЫЕ ПОЛЯ ===
...
void MainWindow::on_btnCreateNewUser_pressed() //создаём нового пользователя
{
    ...
    //user.login()
    //QMessageBox::information(this, "", "Логин: " + newUser.login());

    // === СТРОКА 2: Валидация + сохранение ===
    if (auto err = checkValidForUser(newUser); !err.isEmpty())
        return QMessageBox::warning(this, "Ошибка", err), void();

    // === СТРОКА 3: Отправка в репозиторий ===
