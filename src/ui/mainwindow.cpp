...
// на будущее - для загрузки большого количества пользователей с минимумом памяти
/*
// ui/mainwindow.cpp
void MainWindow::loadUsersPage(int page)
{
    UserFilter filter;
    filter.loginContains = ui->lineEditSearch->text();

    Pagination pagination;
    pagination.page = page;
    pagination.pageSize = 50;

    auto result = m_repo->findFiltered(filter, pagination);

    if (result.ok) {
        updateTableModel(result.value);  // Обновляем QTableView

        // Обновляем пагинатор
        auto countResult = m_repo->countFiltered(filter);
        if (countResult.ok) {
            int totalPages = (countResult.value + pagination.pageSize - 1) / pagination.pageSize;
            ui->paginationWidget->setTotalPages(totalPages);
        }
    }
}
*/
void MainWindow::on_btnCreateNewUser_pressed() //создаём нового пользователя
{
    ...
    for (int i = 0; i < meta->propertyCount(); ++i) {
        ...
        qDebug() << "Свойство:" << prop.name();  // "login"
        qDebug() << "Колонка в БД:" << prop.userType();  // "login"

        // Прочитать значение без знания типа на этапе компиляции!
        QVariant value = prop.read(&user);  // Вернёт QVariant с данными
    }

//где то впихнуть чтение виджетов

    //проверка валидности полей
    login.isEmpty(), email.contains("@")

    //проверка заполнения обязательных полей
    User::isValid() Целостность объекта id > 0 && !login.isEmpty()

    //
    UserRepository* m_repo;
    bool registr = UserRepository::createUser(const User& user); //как лучше, так или через try? и почему

    auto result = m_repo->insert(user);

    if (result.ok) {
        QMessageBox::information(this, "Успех", "ID: " + QString::number(result.value));
        accept();
    } else {
        // Разделяем ошибки по типу
        for (const auto& err : result.errors) {
            if (err.type == ErrorInfo::Validation) {
                // Подсвечиваем поле в форме
                highlightField(err.field, err.message);
            } else if (err.type == ErrorInfo::Database) {
                // Пишем в лог, пользователю — общее сообщение
                qCritical() << "DB Error:" << err.message;
