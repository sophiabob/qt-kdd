...
void MainWindow::initFieldMaps()
{
    m_userFormFields = {  // ← Присваиваем, а не объявляем заново
        ...
        {"lastUpdate",     ui->dateUserLastUpdate,  FieldRecord::Text},
        // без lastUpdate
    };
}
...
void MainWindow::formToUser(User& user) const
{
    ...
    for (const auto& field : m_userFormFields) {
        ...
        //qDebug() << "Processing:" << field.fieldName << "Type:" << field.type;

        if (!field.widget) {
            qDebug() << "  Skip: widget is null";
            continue;
        }

        QVariant value;
        bool valueSet = false;  // ← Флаг: удалось ли прочитать значение
        ...
        switch (field.type) {
            ...
            case FieldRecord::Int:
                if (auto* spin = qobject_cast<QSpinBox*>(field.widget)) {
                    value = spin->value(); valueSet = true;
    ...
    //QString UserLastUpdate = Helpers::timeDateMoscow();
            ...
            case FieldRecord::Label:
                qDebug() << "  ComboBox check:" << field.fieldName
                         << "widget:" << field.widget
                         << "className:" << (field.widget ? field.widget->metaObject()->className() : "nullptr");
                ...
                break;
            default:
                qDebug() << field.fieldName << "  Unknown type:" << field.type;
                continue;
        }

        if (!valueSet) {
            qDebug() << "  Skip: value not set for" << field.fieldName;
        ...
        if (field.fieldName == "login") user.setLogin(value.toString().trimmed().toLower());
        else if (field.fieldName == "password") user.setPasswordHash(value.toString());  // ← Хэшируем! //user.setPasswordHash(hashPasswordSecure(value.toString()));  и в user.h
        else if (field.fieldName == "surname") user.setSurname(value.toString());
        else if (field.fieldName == "firstName") user.setFirstName(value.toString());
        else if (field.fieldName == "patronymic") user.setPatronymic(value.toString());
        else if (field.fieldName == "snils") user.setSnils(value.toInt());
        else if (field.fieldName == "employeeNumber") user.setEmployeeNumber(value.toInt());
        else if (field.fieldName == "role") user.setRole(value.toString());
        else if (field.fieldName == "department") user.setDepartment(value.toString());
        else if (field.fieldName == "cardId") user.setCardId(value.toString());
        else if (field.fieldName == "accessCode") user.setAccessCode(value.toString());
        else if (field.fieldName == "blockReason") user.setBlockReason(value.toString());
        else if (field.fieldName == "dosimetrTldId") user.setDosimetrTldId(value.toInt());
        else if (field.fieldName == "lastCellUpdate") user.setLastCellUpdate(value.toDateTime());
        else if (field.fieldName == "birthDate") user.setBirthDate(value.toDateTime());
        else if (field.fieldName == "annualDose") user.setAnnualDose(value.toFloat());
        else if (field.fieldName == "currentYearDose") user.setCurrentYearDose(value.toFloat());
        else if (field.fieldName == "currentYearDosePPD") user.setCurrentYearDosePPD(value.toFloat());
        else if (field.fieldName == "startDoz") user.setStartDoz(value.toInt());
...
// хэширование пароля
QString MainWindow::hashPasswordSecure(const QString& password) const
{
...
QString MainWindow::checkValidForUser(const User& user) const
{
    ...
    if (user.login().length() < 3) return "Логин: минимум 3 символа";
    if (user.login().length() > 30) return "Логин: максимум 30 символов";
    if (!QRegularExpression("^[a-zA-Z0-9_]+$").match(user.login()).hasMatch())
        return "Логин: только латиница, цифры, _";

    //if (user.passwordHash().length() < 32) return "Пароль: минимум 8 символов";  // SHA-256 = 64 hex
    if (user.passwordHash().length() < 8) return "Пароль: минимум 8 символов";  // SHA-256 = 64 hex

    if (user.snils() > 0) {
        QString s = QString::number(user.snils());
        if (s.length() != 11) return "СНИЛС: ровно 11 цифр";
        //if (!validateSnilsChecksum(s)) return "СНИЛС: неверная контрольная сумма";
    }

    if (user.employeeNumber() < 0) return "Табельный номер: не может быть отрицательным";

    if (user.birthDate().isValid()) {
        if (user.birthDate().date() > QDate::currentDate()) return "Дата рождения: не может быть в будущем";
        if (user.birthDate().date() < QDate(1900, 1, 1)) return "Дата рождения: слишком ранняя";
    }

    if (user.annualDose() < 0) return "Годовая доза: не может быть отрицательной";
    if (user.currentYearDose() < 0) return "Текущая доза: не может быть отрицательной";
    if (user.currentYearDosePPD() < 0) return "Доза ППД: не может быть отрицательной";
    if (user.annualDose() > 0 && user.currentYearDose() > user.annualDose())
        return "Дозиметрия: текущая доза не может превышать годовую";

    if (user.startDoz() > 0 && user.finishDoz() > 0 && user.finishDoz() < user.startDoz())
        return "Дозиметрия: дата окончания не может быть раньше даты начала";
...
{
    User newUser;              // Объект на стеке
    formToUser(newUser); // Заполняем по ссылке
    //qDebug() << "newUser.passwordHash" << newUser.passwordHash();

    if (QString err = checkValidForUser(newUser); !err.isEmpty())
        return QMessageBox::warning(this, "Ошибка", err), void();
    ...
    QString messValid = checkValidForUser(newUser);
    if (!messValid.isEmpty()){
        QMessageBox::critical(this, "ошибка", messValid);
        return;
    }

    qDebug() << "User* newUser" << messValid;
   ...
   // qDebug() << "newUser->fillFromMap";
    ...
    QString rawPassword = ui->inputPassword->text();
    QString hashedPassword = hashPasswordSecure(rawPassword);
    ...
    // User → Репозиторий
    auto result = m_repo->createUser(newUser); //тут проблема
...
void MainWindow::on_pushButton_usersHistoryFile_pressed()
{
    ...
    // Теперь передаем таблицу в функции
    if (format == "csv") {
...
bool MainWindow::saveTableToCSV(QTableWidget *table, const QString &filePath)
{
    if (!table) {
    ...
    for (int col = 0; col < columnCount; ++col) {
        ...
        QString headerText = header ? header->text() : QString("Column%1").arg(col + 1);
        stream << "\"" << headerText.replace("\"", "\"\"") << "\"";
...
{
    // Получаем таблицу из UI
    QTableWidget *table = ui->tableWidget_historyDuty_2;
    if (!table) {
        QMessageBox::critical(this, "Ошибка", "Таблица дежурных пользователей не найдена");
        return;
    }
    ...
    if (format == "csv") {
        defaultFileName = QString("dutyUser-%1.csv").arg(currentDate);
    } else {
        defaultFileName = QString("duty_users_%1.%2").arg(currentDateTime).arg(format);
    }
    ...
    // Используем те же функции экспорта, но передаем другую таблицу
    if (format == "csv") {
        success = saveTableToCSV(table, filePath);
        resultMessage = "CSV файл успешно создан";
    } else if (format == "xlsx") {
...
{
    // Получаем таблицу из UI
    QTableWidget *table = ui->tableWidget_historyDuty;
    if (!table) {
        QMessageBox::critical(this, "Ошибка", "Таблица истории дежурств не найдена");
        return;
    }
    ...
    QString defaultFileName;
    if (format == "csv") {
        defaultFileName = QString("duty-%1.csv").arg(currentDate);
    } else {
        defaultFileName = QString("duty_history_%1.%2").arg(currentDateTime).arg(format);
    }
    ...
    // Используем те же функции экспорта, передавая нужную таблицу
    if (format == "csv") {
        success = saveTableToCSV(table, filePath);
        resultMessage = "CSV файл успешно создан";
    } else if (format == "xlsx") {
...
{
    // Получаем таблицу из UI
    QTableWidget *table = ui->tableWidget_historySet;
    if (!table) {
        QMessageBox::critical(this, "Ошибка", "Таблица истории настроек не найдена");
        return;
    }
    ...
    QString defaultFileName;
    if (format == "csv") {
        // Сохраняем оригинальный формат имени для CSV
        defaultFileName = QString("set-%1.csv").arg(currentDate);
    } else {
        defaultFileName = QString("settings_history_%1.%2").arg(currentDateTime).arg(format);
    }
...
{
    // Получаем таблицу из UI
    QTableWidget *table = ui->tableWidget_historyKas;
    if (!table) {
        QMessageBox::critical(this, "Ошибка", "Таблица истории КАС не найдена");
        return;
    }
    ...
    if (format == "csv") {
        // Сохраняем оригинальный формат имени для CSV
        defaultFileName = QString("kas-%1.csv").arg(currentDate);
    } else {
        defaultFileName = QString("kas_history_%1.%2").arg(currentDateTime).arg(format);
    }
...
{
    // Получаем таблицу из UI
    QTableWidget *table = ui->tableWidget_historyMesh;
    if (!table) {
        QMessageBox::critical(this, "Ошибка", "Таблица истории Mesh не найдена");
        return;
    }
    ...
    if (format == "csv") {
        // Сохраняем оригинальный формат имени для CSV
        defaultFileName = QString("mesh-%1.csv").arg(currentDate);
    } else {
        defaultFileName = QString("mesh_history_%1.%2").arg(currentDateTime).arg(format);
    }
...
{
    std::vector<FileRecord> formatFile = {{
        {"CSV (*.csv)", "csv"},
        {"Excel (*.xlsx)", "xlsx"},
        {"Word (*.doc)", "doc"},
    }};
    QString format = Helpers::dialogWithVariatives(this, "Выбор формата файла?", "Выберите формат для сохранения:", formatFile);
    ...
    if (format == "csv") {
        // Сохраняем оригинальный формат имени для CSV
        defaultFileName = QString("dose_ppd-%1.csv").arg(currentDate);
    } else {
        defaultFileName = QString("dose_history_%1.%2").arg(currentDateTime).arg(format);
    }
    //QString fileFilter = formatName + ";;All Files (*.*)";
    QString filePath = QFileDialog::getSaveFileName(this,
                                                   QString("Сохранить как %1").arg(format),
                                                   QDir::homePath() + "/" + defaultFileName,
                                                   format);
    if (filePath.isEmpty()) {
        return;
    ...
    // Получаем таблицу из UI
    QTableWidget *table = ui->tableWidget_DoseHistory;
    if (!table) {
        QMessageBox::critical(this, "Ошибка", "Таблица истории дозирования не найдена");
        return;
    }
    ...
    if (success) { //данные о файле, если всё сохранилось
        QFileInfo fileInfo(filePath);
        QString successText = QString("%1\n\n"
                                      "Файл: %2\n"
                                      "Путь: %3\n"
...
{
    std::vector<FileRecord> helpVideos = {{
        // Пользователи
        {"пользователи - пользователями", "help_users.mp4"},

        // Наряды
        {"наряды - наряды", "help_idkDutyDuty.mp4"},
        {"наряды - пользователи", "help_idkDutyUsersduty.mp4"},

        // Дозы
        {"дозы - ппд - дозы", "help_idkDosePpdDose.mp4"},
        {"дозы - тлд - дозы", ""},  // пустой путь
        {"дозы - сич - дозы", ""},  // пустой путь
        {"дозы - ожидаемые дозы - дозы", ""},  // пустой путь

        // Отчёты в дозах (пустые)
        {"дозы - отчёты - ппд", ""},
        {"дозы - отчёты - тлд", ""},
        {"дозы - отчёты - сич", ""},
        {"дозы - отчёты - ожидаемые дозы", ""},

        // Эксплуатация (все пустые)
        {"эксплуатация - ппд", ""},
        {"эксплуатация - тлд", ""},
        {"эксплуатация - ппдрд", ""},

        // СИСТЕМЫ ХРАНЕНИЯ - 2
        {"увд гамма - комплекты", "help_uvdSet.mp4"},
        {"увд гамма - блоки хранения", "help_uvdSet.mp4"},
        {"увд гамма - ячейки", "help_uvdSet.gif"},

        {"кид - комплекты", "help_uvdSet.mp4"},
        {"кид - блоки хранения", "help_uvdSet.mp4"},
        {"кид - ячейки", "help_uvdSet.mp4"},

        // ИСТОРИЯ - 4
        {"история - пользователи", "help_historyUsers.mp4"},

        {"история - нарядов - наряды", "help_historyDuty.mp4"},
        {"история - нарядов - наряды пользователей", "help_historyDuty.mp4"},
        {"история - нарядов - дозы", "help_historyDuty.mp4"},

        {"история - комплектов - комплекты", "help_historySet.mp4"},
        {"история - комплектов - блоки хранения", "help_historySet.mp4"},
        {"история - комплектов - ячейки", "help_historySet.mp4"},

        {"история - кид", ""},  // пустой путь
    ...
    QString filePath = Helpers::dialogWithVariatives(this, "Где возникла проблема?", "Выберите вариант из списка", helpVideos);

    if (!filePath.isEmpty()){
        Helpers::playVideoInWindow(this, "help/"+filePath);
    }
}
...
{
    // Получаем таблицу из UI
    QTableWidget *table = ui->tableWidget_historySet_kid;
    if (!table) {
        QMessageBox::critical(this, "Ошибка", "Таблица истории настроек KID не найдена");
        return;
    }
    ...
    if (format == "csv") {
        // Сохраняем оригинальный формат имени для CSV
        defaultFileName = QString("set_kid-%1.csv").arg(currentDate);
    } else {
        defaultFileName = QString("settings_kid_history_%1.%2").arg(currentDateTime).arg(format);
    }
...
{
    // Получаем таблицу из UI
    QTableWidget *table = ui->tableWidget_historyKas_kid;
    if (!table) {
        QMessageBox::critical(this, "Ошибка", "Таблица истории КАС KID не найдена");
        return;
    }
    ...
    if (format == "csv") {
        // Сохраняем оригинальный формат имени для CSV
        defaultFileName = QString("kas_kid-%1.csv").arg(currentDate);
    } else {
        defaultFileName = QString("kas_kid_history_%1.%2").arg(currentDateTime).arg(format);
    }
...
{
    // Получаем таблицу из UI
    QTableWidget *table = ui->tableWidget_historyMesh_kid;
    if (!table) {
        QMessageBox::critical(this, "Ошибка", "Таблица истории Mesh KID не найдена");
        return;
    }
    ...
    if (format == "csv") {
        // Сохраняем оригинальный формат имени для CSV
        defaultFileName = QString("mesh_kid-%1.csv").arg(currentDate);
    } else {
        defaultFileName = QString("mesh_kid_history_%1.%2").arg(currentDateTime).arg(format);
    }
